$(document).ready(function () {
    ...
    //    var formularioRevisado = $('#checkFormulario').is(':checked');
    //    var multasRevisadas = $('#checkMultas').is(':checked');

    //    if (formularioRevisado && multasRevisadas) {
    //        // Activar el botón de confirmar caso y el combo de radicadores
    //        $('#btnConfirmarCaso').prop('disabled', false);
    //        $('#selecRadicador').prop('disabled', false);
    //    } else {
    //        // Desactivar el botón de confirmar caso y el combo de radicadores
    //        $('#btnConfirmarCaso').prop('disabled', true);
    //        $('#selecRadicador').prop('disabled', true);
    //    }

    //    // Desactivar los radio buttons relacionados si los checks están marcados
    //    $('input[name="radioRevision"], input[name="radioRevisionmulta"]').prop('disabled', formularioRevisado && multasRevisadas);
    //}

    //// Desactivar por defecto el combo de doctores y el botón de confirmar
    ...
    //$('#checkFormulario, #checkMultas').change(function () {
    //    verificarCondiciones();
    //});

    //function obtenerDoctorAsignado() {
    //    var doctorAsignado = $('#doctorAsignado').text().trim();
    //    if (doctorAsignado) {
    //        $('#selecDoctor').val(doctorAsignado).prop('disabled', true);
    //        $('#btnAsignarDoctor').prop('disabled', true);
    //    }
    //}

    //// Ocultar el label de doctor asignado y los checks en asignación
    //$('#labelDoctorAsignado').hide();
    //$('#checkFormulario, #checkMultas').hide();
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                jQuery.ajax({
                    ...
                    contentType: "application/json; charset=utf-8",
                    success: function (data) {
                        console.log('Solicitud DELETE exitosa:', dadocta);
                        if (data.success) {
                            obtenerArchivo(pgId);
                            Swal.fire({
                                title: "Eliminado!",
    ...
    function validarNombre(pos) {
        var descripcion = "";
        if (pos == 0)
            descripcion = "Foto de su licencia Vencida";
        else if (pos == 1)
            ...
            description = "Copia de planilla o W2";
        else if (pos == 10)
    ...
    function llenarhtml(url, name, id, pos) {
        console.log('data', pos, ':::::::::::::::::::::::::::::', url, '::', name, ':::', id);
    ...
    function validarPDF() {
        ...
        jQuery.ajax({
            url: 'https://api.tulicenciapr.com/api/Archivo/' + $('#frm_id').val(),
            type: "GET",
            data: null,
            dataType: "json",
            contentType: "application/json; charset=utf-8",
    ...
    $('#btnVerPDF').on('click', function () {
        ...
        var trId = $('#tr_id').val();
        var frmId = $('#frm_id').val();
        var url = baseApiUrlEndPoint + '/pdf/gen-consolidado/' + trId + '/' + frmId;
        var urlVisor = urlPdfViewer + '?url=' + url + '/1';
    ...
    $('#btnGrabarPDF').on('click', function (e) {
        ...
        var rutaHTML = 'https://tulicenciapr.com/admin/Home/VerFormPDFRenovacion/' + cl_id + '/' + tr_id;
        ...
        jQuery.ajax({
            url: 'https://api.tulicenciapr.com/api/Archivo/caso1Upload',
            type: 'POST',
    ...
    // ASIGNACION
    jQuery.ajax({
        url: 'https://api.tulicenciapr.com/api/Administrador/Radicadores',
        type: "GET",
        data: null,
        dataType: "json",
        contentType: "application/json; charset=utf-8",
