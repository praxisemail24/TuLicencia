...
$(document).ready(function () {
    ...
    $('#btnVerPDF').on('click', function () {
        ...
        //$('#btnImprimirForm').attr('data-hint', baseApiUrlEndPoint + '/pdf/consolidado/' + trId + '/' + frmId);
        $('#btnImprimirForm').attr('data-print', baseApiUrlEndPoint + '/pdf/consolidado/' + tr_id + '/' + frm_id);
        $('#modalPdf').modal('show');
    });
...
async function downloadPDF2(imagenes, tramite, rutapdf, cl_id, tr_id) {
    ...
    doc.text("Licencia de Reciprocidad", 52, 50, { align: "left" });
    ...
    doc.text("• Fecha de nacimiento y Presencia Legal ", 25, 110, { align: "left" });
    doc.text("• Foto con ID", 25, 120, { align: "left" });
    doc.text("• Recibo de luz y agua  ", 25, 130, { align: "left" });
