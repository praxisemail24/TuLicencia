document.addEventListener('DOMContentLoaded', function () {
    ...
    var modalElement = document.getElementById('modalPdf');
...
$(document).ready(function () {
    ...
    jQuery.ajax({
        ...
        success: function (data) {
            ...
            //$("#ddlPueblos1").val(item.pueblo);
            //$("#ddlPueblos1").val(item.pueblo).next('.select2-container').show();
            $("#ddlPueblos1").val(item.pueblo).trigger('change');

            //$('#ddlPueblos1').next('.select2-container').show();
    ...
    $('#divImagen').on('click', 'a.btn-eliminar', function (e) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                jQuery.ajax({
                    url: 'https://api.tulicenciapr.com/api/Archivo/' + ar_id,
                    type: "DELETE",
                    dataType: "json",
                    contentType: "application/json; charset=utf-8",
                    success: function (data) {
    ...
    function ActualizarFormulario() {
        ...
        jQuery.ajax({
            ...
            //url: 'https://localhost:7080/api/RenovLic/actualizarDatosForm/',
        ...
        jQuery.ajax({
            ...
            success: function (data) {
                ...
                if (data.items.length === 0) {
                    llenarhtml(null, validarNombre(0), 0, 0);
                    llenarhtml(null, validarNombre(2), 0, 2);
                    ...
                    item = data.items.find(t => t.ar_pos == 0);
                    if (item) {
                        llenarhtml(item.ar_nombre, validarNombre(item.ar_pos), item.ar_id, item.ar_pos);
                    }
                    else {
                        llenarhtml(null, validarNombre(0), 0, 0);
                    }
    ...
    function validarNombre(pos) {
        ...
        if (pos == 0)
            ...
            descripcion = "Copia de planilla o W2";
        else if (pos == 10)
    ...
    //    var url = baseApiUrlEndPoint + '/pdf/gen-consolidado/' + tr_id + '/' + frm_id + '/1';
    //    var urlVisor = urlPdfViewer + '?url=' + url;
    //    $('#ihtml').attr('src', urlVisor);
    //    $('#pdfPreview').attr('src', urlVisor);
    //    $('#btnDescargarPDF').attr('href', baseApiUrlEndPoint + '/pdf/consolidado/' + tr_id + '/' + frm_id);
    //    console.log('URL::::::::::::', url);
    //    $('#btnImprimirForm').attr('data-print', baseApiUrlEndPoint + '/pdf/consolidado/' + tr_id + '/' + frm_id);
...
async function downloadPDF2(imagenes, tramite, rutapdf, cl_id, tr_id) {
    ...
    doc.text("Renovación de Licencia", 52, 50, { align: "left" });
    ...
    doc.text("• Foto de su licencia de conducir vencida ", 25, 100, { align: "left" });
    doc.text("• Foto ID ", 25, 110, { align: "left" });
    doc.text("• Copia de planilla o W2 ", 25, 120, { align: "left" });
    doc.text("• Recibo de agua y/o luz  ", 25, 130, { align: "left" });
